 ...
 *   getPolynom()      => null
 */
function getPolynom(...elems) {
  if (elems.length === 1) return () => elems[0];
  if (elems.length === 2) return (x) => elems[0] * x + elems[1];
  if (elems.length === 3)
    return (x) => elems[0] * x ** 2 + elems[1] * x + elems[2];
  return null;
}
...
function logger(func, logFunc) {
  return (...elems) => {
    const elemString = elems.map((el) => JSON.stringify(el)).join(',');

    logFunc(`${func.name}(${elemString}) starts`);
    const outcome = func(...elems);
    logFunc(`${func.name}(${elemString}) ends`);
